...
import numpy as np
from rere_dml import Triplet as Trip


# _device = torch.device("cuda" if torch.cuda.is_available() else "cpu")


class NystromNet(torch.nn.Module):
    ...
    def __init__(self, d_in, samples, d_out, if_dml_reg=False, if_fix_nystrom=False, kernel_function='rbf'):
        super(NystromNet, self).__init__()
        self._d_in = d_in
    ...
    def encode_dml(self, x):
        # xs = x.view(-1, self._d_in)
        # xs = F.relu(self.dml1(x))
        # xs = self.dml2(xs)
    ...
    def encode_nys(self, x):
        ...
        nys_in = self.compute_whole_nys(x, sam_new)
